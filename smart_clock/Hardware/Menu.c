...
uint16_t Menu2_Clock()
{
	...
	OLED_ShowString(0,48,"数据            ",OLED_8X16);
				if(DirectFlag==1)
				{
					OLED_Animation( 0,16,64,16,0,0,16,16 );
				}
				if(DirectFlag==2)
				{
...
uint16_t Menu2_Setting()
{
	...
	OLED_ShowString(0,48,"语言              ",OLED_8X16);
			...
			case 5:{
	OLED_ShowString(0,0,"模式            ",OLED_8X16);
	OLED_ShowString(0,16,"动画设置          ",OLED_8X16);
	OLED_ShowString(0,32,"语言              ",OLED_8X16);
	OLED_ShowString(0,48,"版本              ",OLED_8X16);
			OLED_ReverseArea(0,48,128,16);
			OLED_Update();
			break;
			}
